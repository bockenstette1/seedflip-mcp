...
function deriveChartTokens(accent: string, bg: string): ChartTokens {
  const [ah, as] = hexToHSL(accent);
  const [, , bl] = hexToHSL(bg);
  const isDark = bl < 50;
  const isMonochrome = as < 8;
  const baseHue = isMonochrome ? 220 : ah;
  const baseSat = isMonochrome ? 45 : Math.max(as, 35);
  const offsets = [0, 50, 140, 200, 280];
  const targetL = isDark ? 65 : 45;
  const chart = offsets.map((off, i) => {
    const hue = (baseHue + off) % 360;
    const sat = i === 0 ? Math.min(baseSat, 70) : Math.max(baseSat - 10, 30);
    return hslToHex(hue, sat, targetL);
  });
  const semL = isDark ? 60 : 42;
  return {
    chart1: chart[0], chart2: chart[1], chart3: chart[2],
    chart4: chart[3], chart5: chart[4],
    success: hslToHex(145, 55, semL),
    warning: hslToHex(38, 70, semL),
    error: hslToHex(0, 65, semL),
  };
}
...
export function formatTokens(seed: DesignSeed): string {
  ...
  const ct = deriveChartTokens(seed.accent, seed.bg);
  return `## SeedFlip — ${seed.name}
> seedflip.co/seed/${seedSlug}
...
| Token | Value | Role |
|-------|-------|------|
| Chart 1 | ${ct.chart1} | Primary data series |
| Chart 2 | ${ct.chart2} | Secondary data series |
| Chart 3 | ${ct.chart3} | Tertiary data series |
| Chart 4 | ${ct.chart4} | Fourth data series |
| Chart 5 | ${ct.chart5} | Fifth data series |
| Success | ${ct.success} | Positive states |
| Warning | ${ct.warning} | Caution states |
| Error | ${ct.error} | Destructive actions |
...
:root {
  ...
  /* Chart */
  --sf-chart-1: ${ct.chart1};
  --sf-chart-2: ${ct.chart2};
  --sf-chart-3: ${ct.chart3};
  --sf-chart-4: ${ct.chart4};
  --sf-chart-5: ${ct.chart5};

  /* Semantic */
  --sf-success: ${ct.success};
  --sf-warning: ${ct.warning};
  --sf-error: ${ct.error};
...
@layer base {
  :root {
    ...
    --ring: ${accent};
    --radius: ${radiusRem};
    --chart-1: ${hexToHSLString(ct.chart1)};
    --chart-2: ${hexToHSLString(ct.chart2)};
    --chart-3: ${hexToHSLString(ct.chart3)};
    --chart-4: ${hexToHSLString(ct.chart4)};
    --chart-5: ${hexToHSLString(ct.chart5)};
    --success: ${hexToHSLString(ct.success)};
    --warning: ${hexToHSLString(ct.warning)};
    --error: ${hexToHSLString(ct.error)};
