 ...
 * SeedFlip MCP Server
 *
 * Gives AI agents access to 100+ curated design seeds.
 * When an agent needs a design direction, it calls SeedFlip
 * instead of guessing colors and fonts.
 *
 * Tools:
 *   get_design_seed  — Get a curated design seed by reference, vibe, or style
...
server.tool(
  'get_design_seed',
  `Get a curated design seed for your project. Returns production-ready design tokens (fonts, colors, spacing, shadows, border radius) with implementation guidance.
...
Returns complete design tokens in your requested format (tokens, tailwind, css, shadcn, or openclaw).`,
  {
    ...
    format: z
      ...
      .optional()
      .describe(
        'Output format. "tokens" = readable summary with all values and design direction. "tailwind" = tailwind.config.ts. "css" = CSS custom properties. "shadcn" = shadcn/ui theme globals.css. "openclaw" = OpenClaw dashboard theme. Defaults to tokens.'
      ),
    count: z
      .number()
      .optional()
